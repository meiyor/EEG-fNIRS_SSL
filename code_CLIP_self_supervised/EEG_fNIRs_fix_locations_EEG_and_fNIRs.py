"""
 ...
 the models evaluated in NeuraPClab

 This module executes the EEG–fNIRS multimodal CLIP-style training, analysis, and evaluation.

 This module implements:
 - Dataset assembly and normalization for EEG and fNIRS trials.
 - Multiple encoders (EEGNet/variants, shallow CNNs) and fusion heads.
 - CLIP-style contrastive training (self- and supervised variants).
 - Fine-tuning for stimulus / subject / HbO-label classification.
 - Probing utilities: feature extraction, t-SNE/UMAP, interpretability
  (Grad-CAM-inspired), and plotting (topomaps, confusion matrices).

 References:
 - Steinmetzger et al. (2022) dataset with simple 1-tone auditory tasks.

 Notes:
 - Reproducibility: seeds and deterministic flags are set but some PyTorch and
   dataloader paths remain non-deterministic depending on transforms and CUDA.
 - Shapes: EEG typically (C_eeg, T_eeg), fNIRS (C_fnirs, T_fnirs). Most models
   expect input as (B, 1, C, T) ** This shapes need to be mantained here for
...
# for the stratified k-fold evaluation just run it one time!!
...
# use the umap for training - this is replicable and more stable, plot
# both tsne and umap in this approach
...
def apply_butter_worth_filter(data: Any, fs: int):
    ...
    # Design the Butterworth filter as bandpass avoid adding distorted phases
    # shifts to the signal
...
def load_matlab_file(path):
    ...
    -------
    dict[str, Any]
       Mapping of variable names to arrays.
...
def load_info_from_file(data_folder: str):
    ...
    ----------
    data_folder : str
      Path to a .mat file with fields:
      - data.label : channel labels
      - data.trialinfo : trial info (only shape used)

    Returns
    -------
    mne.Info
        EEG info with channel names, sfreq=256 Hz, and montage set.

    Notes
    -----
    - Removes ocular channels (HEOG/VEOG) and normalizes 'PO7''/ 'PO8''.
    - Uses 'standard_1005' electrode positions and subsets them to present
       channels only.
    """
...
def plotting_relevance_map_eeg_fnirs(eeg_xai_channels, fnirs_xai_channels, info, info_fnirs, epoch, folder_imgs):
    """
    Save EEG and fNIRS topomap relevance visualizations.

    Parameters
    ----------
    eeg_xai_channels : np.ndarray, shape (n_eeg_channels, n_time)
       Channel-wise EEG relevance values over time.
    fnirs_xai_channels : np.ndarray, shape (n_fnirs_channels,)
       Aggregated channel-wise fNIRS relevance values.
    info : mne.Info
       EEG info (with montage) matching `eeg_xai_channels`.
    info_fnirs : mne.Info
       fNIRS info (with montage), concatenated [HbO (0:38), HbR (38:76)].
       After removing the corresponding channels.
    epoch : int
       Epoch index for titles/filenames.
    folder_imgs : str
    ...
    -------
    None
       Files are written to `folder_imgs`.

    Notes
    -----
    - Uses private MNE API '_find_topomap_coords'; subject to change in case this is need it,
      and prevent problems in montage changes
    - Saves multiple time-windowed EEG maps and HbO/HbR fNIRS maps.
    """
    ...
    # plot here the fnirs representation (just the whole one...)
    info_hbo = mne.pick_info(info_fnirs, np.arange(0, 38, 1))
    info_hbr = mne.pick_info(info_fnirs, np.arange(38, 76, 1))

    fig, ax = plt.subplots(figsize=(10, 10))
    im_fnirs_hbo, _ = mne.viz.plot_topomap(
        fnirs_xai_channels[0:38],
        info_hbo,
        axes=ax,
        vlim=(0, 1),
        sensors=False,
    ...
    # Get 2D coordinates of sensors
    pos = mne.channels.layout._find_topomap_coords(info_hbo, None)
    # Draw bigger dots
    ax.scatter(pos[:, 0], pos[:, 1], s=150, c="black", zorder=10)

    clb = plt.colorbar(im_fnirs_hbo, ax=ax, orientation="horizontal", fraction=0.05)
    ...
    for text in ax.texts:
        text.set_fontsize(16)  # <-- increase font size her
    plt.suptitle(f"Epoch {epoch}", fontsize=20)
    plt.savefig(f"{folder_imgs}/relevance_measure_fnirs_hbo_{epoch}.jpg", dpi=300)
    plt.close("all")

    fig, ax = plt.subplots(figsize=(10, 10))
    im_fnirs_hbr, _ = mne.viz.plot_topomap(fnirs_xai_channels[38:], info_hbr, axes=ax, vlim=(0, 1), sensors=False, cmap="RdBu_r", extrapolate="local", sphere=(0, 0, 0, 0.05), show=False)

    # Get 2D coordinates of sensors
    pos = mne.channels.layout._find_topomap_coords(info_hbr, None)

    # Draw bigger dots
    ax.scatter(pos[:, 0], pos[:, 1], s=150, c="black", zorder=10)

    clb = plt.colorbar(im_fnirs_hbr, ax=ax, orientation="horizontal", fraction=0.05)
    ...
    for text in ax.texts:
        text.set_fontsize(16)  # <-- increase font size her
    plt.suptitle(f"Epoch {epoch}", fontsize=20)
    plt.savefig(f"{folder_imgs}/relevance_measure_fnirs_hbr_{epoch}.jpg", dpi=300)
    plt.close("all")


def init_weights_uniform(m):
    """
    Initialize Conv2d/Linear weights uniformly in [-0.1, 0.1]; zero biases.

    Parameters
    ----------
    m : nn.Module
      Module passed to `apply`.
...
def init_weights_xavier(m):
    """
    Initialize Conv2d/Linear weights with Xavier uniform; zero biases.
...
def normalization_trial_minmax(x):
    """
    Min-max normalize a single trial to [0, 1].
    ...
    Notes
    -----
    - If max==min, denominator is zero. Consider adding epsilon in practice.
    """
...
def normalization_trial_zscore(x):
    """
    Z-score normalize a single trial.
    ...
    -------
    np.ndarray
       (x - mean) / std. If std==0, output is NaN; guard in production.
    """
    mean = x.mean()
    std = x.std()
    return (x - mean) / (std)
...
# define random erase across time with random ROIs patches FC_R1, FC_R2,
# CP_1, CP_2, P_1, P_2, O_1 with overlap. This is defined for a coarse
# grained ROI-based occlusion methods.


class RandomErasingOneROIs:
    """
    Randomly erase a temporal window across a pre-defined ROI subset of EEG channels.
    ...
    min_frac, max_frac : float
       Min and max fraction of the time-length to erase.
    value : float
       Fill value for erased region.
    channels : list[str]
       EEG channel name list used to locate ROI indices.

    Call Signature
    --------------
    __call__(x: torch.Tensor) -> torch.Tensor
     x shape (B, C, T). Returns modified tensor.

    Raises
    ------
    ValueError
       If a required channel name is missing from `channels`.
    """
    ...
    def __call__(self, x):
        ...
        # do here the randome erasing for EEG on individual ROI
...
class RandomErasingOneROIsMoreGranular:
    """
    Randomly erase multiple small ROIs (3 picked) over time on EEG channels.

    Parameters
    ----------
    p, min_frac, max_frac, value, channels
       Same meaning as RandomErasingOneROIs.
       but with more granularity. Do three occlusions at a time!

    Returns
    -------
    torch.Tensor
       Tensor with erased temporal windows across selected ROIs.
    """
    ...
    def __call__(self, x):
        ...
        # do here the random erasing with the fine-grained occlusion across EEG image..
...
class RandomErasingOneDimPatch:
    """
    Randomly erase a contiguous patch along 'height' (channels) or 'width' (time).

    Parameters
    ----------
    erase_along : {'height','width'}
    p : float
    min_frac, max_frac : float
       Fraction of the selected dimension to erase.
    value : float

    Returns
    -------
    torch.Tensor
        Tensor with erased region.
    """
...
def plot_confusion_matrix_test(labels: Any, preds: Any, labels_disp: Any, suffix: str, title: str, metrics: Any, folders: str):
    """
    Plot and save a confusion matrix with counts and overall percentages.
    Parameters
    ----------
    labels : array-like
        Ground-truth labels (ints).
    preds : array-like
        Predicted labels (ints).
    labels_disp : sequence[str]
        Class names for display.
    suffix : str
        Filename suffix (e.g., fold/epoch id).
    title : str
        Plot title prefix.
    metrics : tuple[float, float, float, float]
        (acc, pr, re, f1) for display in title.
    folders : str
        Output directory.
    ...
    -------
    None
        Image saved as 'conf_matrix_epoch_fold_{suffix}.jpg'.

    Notes
    -----
    - Percentages are of total trials. For per-class rates, normalize rows.
    """
...
def plot_tsne_feat(tsne_feat, labels, title: str, epoch: str, suffix: str, folder_name: str, class_value: str, subj: str):
    ...
    Parameters
    ----------
    tsne_feat/ umap_feat : np.ndarray, shape (N, 2) -> this can be tsne_feat or umap_feat input representations
    labels : array-like
...
class CLIPDataset(Dataset):
    ...
    def __init__(self, eeg, fnirs, labels, subject_labels, labels_HbO, normalization_function):
    ...
    def __getitem__(self, idx):
        ...
        return eeg_tensor, fnirs_tensor, labels_output, labels_subject, labels_hbo
...
class EEGNet(nn.Module):
    """
    EEGNet-style encoder for EEG, with attention head.

    Parameters
    ----------
    n_channels : int, default=43
    embed_dim : int, default=128
    n_times : int, default=548
    dropout, F1, D, F2 : hyperparameters

    Returns
    -------
    (embedding, flattened_conv_features)
    """
...
# separate the attentional heads only if this is necessary for doing fine-tuning -- not in this time.. but you can replicate with the excerpts of the codes below...
class AttHead(nn.Module):
    """
    Linear projection followed by multihead self-attention + residual LN.
...
class EEGNetFineTuned(nn.Module):
    """
    Fine-tuning head combining EEG and fNIRS features for classification.

    Parameters
    ----------
    EEGNet_extractor : nn.Module
       EEG feature extractor with attribute `flatten_dim`.
    fNIRsNet_extractor : nn.Module
       fNIRS feature extractor with attribute `flatten_dim`.
    embed_dim : int, default=128 -> Get this from the feature extractor previously defined
    label_dim : int, default=2 -> Get this from the problem you want to solve in the fine-tuning part...
       Number of classes.

    Forward
    -------
    forward(x_eeg, x_fnirs) -> torch.Tensor
        Logits of shape (B, label_dim).
    """
    ...
    def __init__(self, EEGNet_extractor, fNIRsNet_extractor, embed_dim=128, label_dim=2):  # replace with the number of labels for initial evaluation
        ...
        # self.flatten = nn.Flatten() # uncomment this if you want to use a non-linear head in the fine-tuning part...
    ...
    def forward(self, x_eeg, x_fnirs):
        ...
        # uncomment this if you want to use the multi-head attentional layer in the forward code..
        # x = self.linear(x_eeg)
        # x = self.flatten(x)
        # attn_output, _ = self.attn(x, x, x)
        # x = self.norm(attn_output + x)
...
# define the ELU classifier here -> ELU works better for this particular case, you can rename if you consider..
class ReLUClassifier(nn.Module):
    """
    Simple ELU MLP classifier over concatenated EEG+fNIRS features.
    ...
    def forward(self, x_eeg, x_fnirs):
        ...
        # don't add this to prevent overfitting.. you can uncomment it if you consider ** only if you consider...
...
def weight_activation(feats, grads):
    """
    Grad-weighted activation map (ReLU(grads) * feats), summed over channels.

    Parameters
    ----------
    feats : torch.Tensor, shape (B, C, ...)
    grads : torch.Tensor, shape (B, C, ...)

    Returns
    -------
    np.ndarray
      CAM-like map per sample, shape collapses channel dim via sum.
    """
...
class GradCAM(nn.Module):
    """
    Grad-CAM-like scorer for cross-modal similarity with hooks on features.

    Parameters
    ----------
    encoder_EEG : nn.Module
    encoder_fNIRs : nn.Module
    attentional_head_eeg : nn.Module
    attentional_head_fnirs : nn.Module

    Forward
    -------
     forward(eeg_img, fnirs_img) -> torch.Tensor
        Cosine similarity score (B,).

    Notes
    -----
    - Hooks store gradients/features in 'self.gradients'/'self.features'.
    - You must call 'score.sum().backward()' (or similar) to populate grads.
    Remove the hooks to make it more reliable in convinience..
    """
    ...
    def forward(self, eeg_img, fnirs_img):
        ...
        # comment this to remove the wooks if you consider it unnecessary
        h1.remove()
        h2.remove()
...
class EEGNetFeatureExtractor(nn.Module):
    """
    EEG feature extractor (EEGNet-like) returning flat conv features and pre-flat tensor.

    Forward
    -------
    forward(x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]
      (flat_features, pre_flat_tensor)

    It is defined based on the layers defined by the feature extractor
    of The EEGNet model defined above. Just the convolutional, batch_norm,
    and pooling layers.
    """
...
class fNIRsNetFeatureExtractor(nn.Module):
    """
    fNIRS feature extractor (EEGNet-like) returning flat conv features and pre-flat tensor.

    Forward
    -------
    forward(x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]
    It is defined based on the layers defined by the feature extractor
    of The fNIRSNet (EEGNet-inspired) model defined above. Just the convolutional, batch_norm,
    and pooling layers.
    """
...
# use the shallow nets defined here if you want to try with smaller encoders
# they will work too for the subject separabilty, but they are not as accurate
# as the EEGNet encoder style..
class ShallowConvNet_EEG(nn.Module):
    """
    Shallow CNN for EEG with two conv blocks and an MLP head.

    Parameters
    ----------
    input_shape : tuple[int, int, int]
        (C, H, W) passed as (channels_in=1?; see usage) to Conv2d.
...
class ShallowConvNet_fNIRs(nn.Module):
    """
    Shallow CNN for fNIRS with two conv blocks and an MLP head.
...
# function for testing the finetune model -> employ this just if you want to employ the finetuning part..
...
# define the training function for the finetuned model ->  employ this just if you want to employ the finetuning part..
def train_finetune_model(finetune_model, train_loader, test_loader, epochs=100, device="cuda", option_training=None, epoch_self=None, lr=1e-3):
    ...
    # consider to change this, only if you consider for the finetuning part..
    scheduler = CosineAnnealingLR(optimizer, T_max=epochs, eta_min=2e-4)
    # scheduler = CosineAnnealingLR(optimizer, T_max=epochs * len(train_loader), eta_min=2e-4)
    ...
    for epoch in range(1, epochs + 1):
        ...
        for eeg_imgs, fnirs_imgs, labels, subject_labels, labels_hbo in tqdm(train_loader, disable=True, desc=f"Epoch {epoch}/{epochs}"):
            ...
            loss.backward()
            optimizer.step()
            # change the location of this step if you desire!
            scheduler.step()
...
# prepare this for testing the model capabilities - use this to get the feature seed for each training, val, test sets
def get_features(encoder_EEG, encoder_fNIRs, dataloader, do_random_occlusion=False, device="cuda", channels=None, interp_option=None):
    """
    ...
    with torch.no_grad():
        for eeg, fnirs, y, subj, hbo in dataloader:
            ...
            z_fnirs, _ = encoder_fNIRs(fnirs_features.unsqueeze(1))

            if do_random_occlusion is True:
                # process here the occluder labels for evaluating the interpretability - as similarity difference..
                z_eeg_occ, _ = encoder_EEG(eeg_features_occluded.unsqueeze(1))
                z_fnirs_occ, _ = encoder_fNIRs(fnirs_features_occluded.unsqueeze(1))

                sims.append(measure(z_eeg_occ, z_fnirs_occ))
                weights_eeg.append(z_eeg_occ.norm(dim=-1))
                weights_fnirs.append(z_fnirs_occ.norm(dim=-1))

            z_eeg_norm = F.normalize(z_eeg, dim=1)
            z_fnirs_norm = F.normalize(z_fnirs, dim=1)

            # append the features projected per batch
    ...
    # calculate here the similarity differences on pairs per batch - validates here if the random occlusion will be done
    if do_random_occlusion is True:
        sim_representation = torch.cat(sims, dim=0)
        baseline = measure(torch.cat(all_features_eeg), torch.cat(all_features_fnirs))
        ...
        return (
            ...
            torch.cat(all_features_fnirs),
            y_all,
            subj_all,
            hbo_all,
            torch.cat(all_eeg_occ),
            torch.cat(all_fnirs_occ),
            sims,
            torch.cat(weights_eeg, dim=0).cpu().numpy(),
            torch.cat(weights_fnirs, dim=0).cpu().numpy(),
        ...
        return torch.cat(all_features_eeg), torch.cat(all_features_fnirs), y_all, subj_all, hbo_all, None, None, None, None, None, None, None, None, None, torch.cat(all_images_eeg), torch.cat(all_images_fnirs)


# define the nearest neighbors clusters for the experiment and add them to the test
def calculate_nearest_subject_neighors(umap_representation: Any, nearest_neighbors: int, subject_labels: Any, index_subj: int, hbo_labels: Any):
    """
    ...
    for label in range(0, 20):
        ...
        points_in_cluster = umap_representation[subject_labels == label]
...
# do a normalization for tsne representation - ONLY IF NECESSARY, this is not used here!!
def normalize_tsne_representation(tsne_selected: Any):
    """
    Min-max normalize each t-SNE dimension to [0,1].

    Parameters
    ----------
    tsne_selected : np.ndarray, shape (N, 2) or (N, d)

    Returns
    -------
    np.ndarray
     Normalized t-SNE coordinates.
    """
...
# set here the function definition for the CLIP - This is the main function for executing the of the EEG+fNIRs CLIP idea!!
def CLIP_train(
    ...
    Train the CLIP-style encoders on EEG–fNIRS with inner subject-out cross-validation, t-SNE/UMAP probes, and classical baselines.

    Parameters
    ----------
    dataset_train : torch.utils.data.Dataset
    test_dataset : torch.utils.data.Dataset
    labels_Subject : np.ndarray
       All subject ids (for split/metrics).
    num_epochs : int
    temperature_ini : float
       Initial temperature (may be scheduled externally).
    folder_name_save, folder_models : str
       Output directories for figs and checkpoints.
    batch_size, embed_dim : int
    info, info_fnirs : mne.Info
        For plotting topomaps/interpretability.
    interp_option : {0,1,2}
        Erasing strategy (see `get_features`).
    learning_rate : float
    subject_start : int
        Starting subject id for outer loop.
    components_tsne : int
        t-SNE components (usually 2).
    n_subs_val : int
        number of subjects to pick as neighbors for validation sampling.
    k_value_self, k_value_class : int
        Log-schedule hyperparams for temperature paths.
    umap_feat : int
        UMAP number of features or n_components.

    Returns
    -------
    None
     This function writes the interim and full results to disk (figures, models, CSVs).
    ...
    for fold_inner, (train_index, val_index) in enumerate(kf.split(dataset_train)):
        for subj in range(subject_start, 20):
            ...
            classifier = ReLUClassifier(input_dim=encoder_EEG.flatten_dim + encoder_fNIRs.flatten_dim, hidden_dim=200, num_classes=20).cuda()  # define the classes for the 20 subjects
            ...
            optimizer = torch.optim.AdamW(list(encoder_EEG.parameters()) + list(encoder_fNIRs.parameters()), lr=learning_rate)
            ...
            # take into account you can change it for the amount of batches if you consider - but for this particular case this use the epochs
            scheduler_classifier = CosineAnnealingLR(optimizer_classifier, T_max=num_epochs, eta_min=5e-5)
            # use this in case you need it or if you want that this doesn't decrease so fast. But for our particular case this workds fine
            scheduler_classifier = CosineAnnealingLR(optimizer_classifier, T_max=num_epochs * len(train_subset), eta_min=5e-5)
            ...
            for epoch in range(0, num_epochs):
                ...
                train_loader = DataLoader(train_subset, batch_size=current_batch_size, shuffle=True, pin_memory=True)
                val_loader = DataLoader(val_subset, batch_size=current_batch_size, shuffle=False, pin_memory=True)
                data_loader_test = DataLoader(test_dataset, batch_size=current_batch_size, shuffle=False, pin_memory=True)
                ...
                for eeg_imgs, fnirs_imgs, labels, subject_labels, labels_hbo in train_loader:
                    ...
                    z_eeg_norm = F.normalize(z_eeg, dim=1)
                    z_fnirs_norm = F.normalize(z_fnirs, dim=1)
                    out_class = classifier(eeg_flat, fnirs_flat)

                    # append the embeddings
                    if z_eeg_norm.shape[0] == current_batch_size:
                        embeddings_eeg.append(z_eeg_norm.detach().cpu().numpy())
                        embeddings_fnirs.append(z_fnirs_norm.detach().cpu().numpy())

                    info_nce_loss = clip_loss(z_eeg_norm, z_fnirs_norm, temperature=temperature_self)
                    info_nce_loss_class = clip_loss_conditional(z_eeg_norm[subject_labels != subj], z_fnirs_norm[subject_labels != subj], labels_hbo.cuda().long()[subject_labels != subj], temperature_class)
                    ...
                    loss_general = (info_nce_loss + info_nce_loss_class) / 2 + 0.01 * class_loss

                    # for now update for each batch!!
                    optimizer.zero_grad()
                ...
                del eeg_features, fnirs_features, z_eeg, z_fnirs, z_eeg_norm, z_fnirs_norm

                # get this after each self-supervised training epoch
                ...
                pretrained_dict_fNIRs = encoder_fNIRs.state_dict()
                feature_extractor_dict_fNIRs = feature_extractor_fNIRs.state_dict()
                ...
                filtered_dict_fNIRs = {k: v for k, v in pretrained_dict_fNIRs.items() if k in feature_extractor_dict_fNIRs}
                feature_extractor_fNIRs.load_state_dict(filtered_dict_fNIRs)
                ...
                att_head_fNIRs = AttHead(flatten_dim=feature_extractor_fNIRs.flatten_dim, embed_dim=embed_dim)

                # get the weights here from the whole encoders
                att_EEG_dict = att_head_EEG.state_dict()
                att_fNIRs_dict = att_head_fNIRs.state_dict()
                ...
                filtered_dict_head_fNIRs = {k: v for k, v in pretrained_dict_fNIRs.items() if k in att_fNIRs_dict}
                att_head_fNIRs.load_state_dict(filtered_dict_head_fNIRs)
                ...
                feat_train_eeg, feat_train_fnirs, train_labels, train_sub_labels, train_hbo_labels, _, _, _, _, _, _, _, _, _, _, _ = get_features(
                    encoder_EEG=encoder_EEG, encoder_fNIRs=encoder_fNIRs, dataloader=train_loader, device="cuda", channels=info.ch_names, interp_option=interp_option
                )
                if interp_option != 3:
                    feat_val_eeg, feat_val_fnirs, val_labels, val_sub_labels, val_hbo_labels, val_eeg_occ, val_fnirs_occ, sims_val, weights_eeg_val, weights_fnirs_val, SIMS_val, SIMS_soft_val, SIM_std_val, SIM_soft_val_std, _, _ = get_features(
                        encoder_EEG=encoder_EEG, encoder_fNIRs=encoder_fNIRs, dataloader=val_loader, do_random_occlusion=True, device="cuda", channels=info.ch_names, interp_option=interp_option
                    )
                    feat_test_eeg, feat_test_fnirs, test_labels, test_sub_labels, test_hbo_labels, test_eeg_occ, test_fnirs_occ, sims_test, weights_eeg_test, weights_fnirs_test, SIMS_test, SIMS_soft_test, SIM_std_test, SIM_soft_test_std, _, _ = get_features(
                        encoder_EEG=encoder_EEG, encoder_fNIRs=encoder_fNIRs, dataloader=data_loader_test, do_random_occlusion=True, device="cuda", channels=info.ch_names, interp_option=interp_option
                    )
                else:
                    feat_val_eeg, feat_val_fnirs, val_labels, val_sub_labels, val_hbo_labels, _, _, _, _, _, _, _, _, _, val_eeg_imgs, val_fnirs_imgs = get_features(
                        encoder_EEG=encoder_EEG, encoder_fNIRs=encoder_fNIRs, dataloader=val_loader, do_random_occlusion=False, device="cuda", channels=info.ch_names, interp_option=None
                    )
                    feat_test_eeg, feat_test_fnirs, test_labels, test_sub_labels, test_hbo_labels, _, _, _, _, _, _, _, _, _, test_eeg_imgs, test_fnirs_imgs = get_features(
                        encoder_EEG=encoder_EEG, encoder_fNIRs=encoder_fNIRs, dataloader=data_loader_test, do_random_occlusion=False, device="cuda", channels=info.ch_names, interp_option=None
                    )

                if interp_option != 3:
                    # compare the sizes of the evaluation => DO THIS FOR EVALUATING INTERPRETABILITY..
                    assert sims_val.shape[0] == val_eeg_occ.shape[0] == val_fnirs_occ.shape[0]
                    EEG_xai = np.zeros((43, 548))  # create image with the size of EEG representation
                    fNIRs_xai = np.zeros((76, 9))  # create image with the size of fNIRs representation
                    weights = list(zip(weights_eeg_val, weights_fnirs_val))
                    ...
                    for n in range(val_eeg_occ.shape[0]):
                        ...
                        fnirs_2d = val_fnirs_occ[n].unsqueeze(0).cpu().numpy().transpose((1, 2, 0)).sum(axis=-1)
                        ...
                        if weight[0] < weight[1]:
                            ...
                            fNIRs_xai[fnirs_2d == 0] += joint_similarity

                else:  # do this for interaction-CAM
                    grad_cam = GradCAM(encoder_EEG=feature_extractor_EEG, encoder_fNIRs=feature_extractor_fNIRs, attentional_head_eeg=att_head_EEG, attentional_head_fnirs=att_head_fNIRs).cuda()
                    score = grad_cam(val_eeg_imgs.unsqueeze(1), val_fnirs_imgs.unsqueeze(1))
                    ...
                    fNIRs_xai = np.mean(cv2.resize(feats_fNIRs, (9, 76)), axis=2)
                ...
                plotting_relevance_map_eeg_fnirs(eeg_xai_channels=EEG_xai, fnirs_xai_channels=np.mean(fNIRs_xai[:, :], axis=1), info=info, info_fnirs=info_fnirs, epoch=epoch, folder_imgs=folder_name_save)
                ...
                # plot the fNIRs image here
                fig, ax = plt.subplots(figsize=(10, 10))
                im = ax.imshow(fNIRs_xai, cmap="jet")
                plt.title(f"fNIRs relevance epoch {epoch}")
                plt.savefig(f"{folder_name_save}/relevance_map_fnirs_{subj}")
                plt.close("all")
                ...
                if (epoch + 1) % 10 == 0 or epoch + 1 == 1:
                    ...
                    # calculate here the wasserstein distance, for training and validation sets
                    wd = wasserstein_distance(
                        np.mean(np.reshape(np.array(embeddings_eeg), (np.array(embeddings_eeg).shape[0] * current_batch_size, embed_dim)), axis=1),
                        np.mean(np.reshape(np.array(embeddings_fnirs), (np.array(embeddings_fnirs).shape[0] * current_batch_size, embed_dim)), axis=1),
                    )
                    wass_dist_vals.append(wd)

                    wd_val = wasserstein_distance(np.mean(feat_val_eeg.detach().cpu().numpy(), axis=1), np.mean(feat_val_fnirs.detach().cpu().numpy(), axis=1))
                    ...
                    nearest_subject_indeces, dist_mean, dist_max = calculate_nearest_subject_neighors(
                        umap_representation=features_UMAP_train[:, :], nearest_neighbors=n_subs, subject_labels=train_sub_labels.detach().cpu().numpy(), index_subj=subj, hbo_labels=train_hbo_labels.detach().cpu().numpy()
                    )
                    ...
                    # EVALUATE HERE THE CLASSIFIERS AFTER PROJECTING THE UMAP FEATURES!!
                    ...
                    if dfs_results_knn["acc_k1"].max() <= acc_knn_hbo_sub_1:
                        ...
                        torch.save(
                            {
                                "epoch": epoch + 1,
                                "model_state_dict": encoder_fNIRs.state_dict(),
                                "optimizer_state_dict": optimizer.state_dict(),
                                "loss": loss_general,
                            },
                            f"{folder_models}/fNIRs_encoder_epoch_{epoch + 1}_{subj}.pth",
                        )
                    ...
                    if epoch + 1 >= 400:  # DO THE EVALUATION UNTIL 400 ITERATIONS**
                        # break the entire execution when reach 400 iterations - as minimum breaking iterations for this evaluation
                        break
            ...
            np.savez_compressed(f"{folder_models}/interpretability_measures_{subj}.npz", EEG_xai=EEG_xai, fNIRs_xai=fNIRs_xai)
...
os.makedirs(
    f"{save_results_path}figs_tsne_{suffix}_{embed_dim}_{batch_size}_{num_epochs}_{learning_rate}_{temperature}_stratified_cross_val_chan_remove_{suffix_interp}_{components_tsne}_{n_subs_val}_{k_value_self}_{k_value_class}_complete_class_subject_OUT_DEF_DEF_mix_more_{feat}_fix_locations",
    exist_ok=True,
)
os.makedirs(
    f"{save_results_path}models_{suffix}_{embed_dim}_{batch_size}_{num_epochs}_{learning_rate}_{temperature}_stratified_cross_val_chan_remove_{suffix_interp}_{components_tsne}_{n_subs_val}_{k_value_self}_{k_value_class}_complete_class_subject_OUT_DEF_DEF_mix_more_{feat}_fix_locations",
    exist_ok=True,
)

folder_name_save = f"{save_results_path}figs_tsne_{suffix}_{embed_dim}_{batch_size}_{num_epochs}_{learning_rate}_{temperature}_stratified_cross_val_chan_remove_{suffix_interp}_{components_tsne}_{n_subs_val}_{k_value_self}_{k_value_class}_complete_class_subject_OUT_DEF_DEF_mix_more_{feat}_fix_locations"
folder_models = f"{save_results_path}models_{suffix}_{embed_dim}_{batch_size}_{num_epochs}_{learning_rate}_{temperature}_stratified_cross_val_chan_remove_{suffix_interp}_{components_tsne}_{n_subs_val}_{k_value_self}_{k_value_class}_complete_class_subject_OUT_DEF_DEF_mix_more_{feat}_fix_locations"
...
# start the folding crossvalition here.. DO THIS EXECUTION ONLY ONE TIME!!
for fold, (train_idx, test_idx) in enumerate(skf.split(EEG_data, labels)):
    ...
    # USE IT FOR NOW WITH ALL THE FREQUENCIES, FILTER IT AFTER THAT TO CONTROL THE SUBJECT CLUSTERING!! JUST UNCOMMENT THIS IF NECESSARY
    ...
    dataset_train = CLIPDataset(eeg=EEG_data_train, fnirs=fNIRs_data_train, labels=labels_train, subject_labels=labels_train_subj, labels_HbO=labels_train_hbo, normalization_function=normalization_trial_minmax)
    dataset_test = CLIPDataset(eeg=EEG_data_test, fnirs=fNIRs_data_test, labels=labels_test, subject_labels=labels_test_subj, labels_HbO=labels_test_hbo, normalization_function=normalization_trial_minmax)
