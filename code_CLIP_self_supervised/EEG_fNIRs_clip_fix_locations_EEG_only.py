        ...
        for subj in range(subject_start, 20):
            ...
            for epoch in range(0, num_epochs):
                ...
                if (epoch + 1) % 10 == 0 or epoch + 1 == 1:
                    ...
                    features_UMAP_train = umap_map.fit_transform(torch.cat((feat_train_eeg, feat_train_eeg_trans), dim=1).detach().cpu().numpy())
                    features_UMAP_val = umap_map.transform(torch.cat((feat_val_eeg, feat_val_eeg_trans), dim=1).detach().cpu().numpy())
                    features_UMAP_test = umap_map.transform(torch.cat((feat_test_eeg, feat_test_eeg_trans), dim=1).detach().cpu().numpy())
