...
def apply_butter_worth_filter(data: Any, fs: int):
    ...
    Parameters:
       data (Any): 1D or 2D array of EEG time-series data.
       fs (int): Sampling frequency in Hz.
       cutoff (float): Cutoff frequency for the low-pass filter (Hz). Default is 15.0.
...
def plotting_relevance_map_eeg_fnirs(fnirs_xai_channels, info_fnirs, epoch, folder_imgs):
    """
...
def get_features(encoder_fNIRs, encoder_fNIRs_trans, dataloader, do_random_occlusion=False, device="cuda", channels=None, interp_option=None):
    """
    ...
    with torch.no_grad():
        for eeg, fnirs, y, subj, hbo, eeg_trans, fnirs_trans in dataloader:
            ...
            fnirs_features_trans = fnirs_trans.cuda()

            # all_images_eeg.append(eeg_features)
            # all_images_fnirs_trans.append(eeg_features)
            all_images_fnirs.append(fnirs_features)
            all_images_fnirs_trans.append(fnirs_features_trans)
            ...
            fnirs_features_occluded_trans = fnirs_features_trans.clone().detach()

            # do the random erasing here for each image
            if do_random_occlusion is True:
                for index_batch in range(0, fnirs_features.size(0)):
                    fnirs_features_occluded[index_batch] = torch.squeeze(random_erase_fnirs(fnirs_features[index_batch].unsqueeze(0)))
                    fnirs_features_occluded_trans[index_batch] = torch.squeeze(random_erase_fnirs(fnirs_features_trans[index_batch].unsqueeze(0)))
                    # fnirs_features_occluded[index_batch] = torch.squeeze(random_erase_fnirs(fnirs_features[index_batch].unsqueeze(0)))
                all_fnirs_occ.append(fnirs_features_occluded)
                all_fnirs_occ_trans.append(fnirs_features_occluded_trans)
                # all_fnirs_occ.append(fnirs_features_occluded)

            z_fnirs, _ = encoder_fNIRs(fnirs_features.unsqueeze(1))
            z_fnirs_trans, _ = encoder_fNIRs_trans(fnirs_features_trans.unsqueeze(1))

            if do_random_occlusion is True:
                z_fnirs_occ, _ = encoder_fNIRs(fnirs_features_occluded.unsqueeze(1))
                z_fnirs_occ_trans, _ = encoder_fNIRs_trans(fnirs_features_occluded_trans.unsqueeze(1))

                sims.append(measure(z_fnirs_occ, z_fnirs_occ_trans))
                weights_fnirs.append(z_fnirs_occ.norm(dim=-1))
                weights_fnirs_trans.append(z_fnirs_occ_trans.norm(dim=-1))

            z_fnirs_norm = F.normalize(z_fnirs, dim=1)
            z_fnirs_norm_trans = F.normalize(z_fnirs_trans, dim=1)

            all_features_fnirs.append(z_fnirs_norm)
            all_features_fnirs_trans.append(z_fnirs_norm_trans)
            # all_features_fnirs.append(z_fnirs_norm)
    ...
    if do_random_occlusion is True:
        ...
        baseline = measure(torch.cat(all_features_fnirs), torch.cat(all_features_fnirs_trans))
        ...
        return (
            ...
            torch.cat(all_features_fnirs_trans),
            y_all,
            subj_all,
            hbo_all,
            torch.cat(all_fnirs_occ),
            torch.cat(all_fnirs_occ_trans),
            sims,
            torch.cat(weights_fnirs, dim=0).cpu().numpy(),
            torch.cat(weights_fnirs_trans, dim=0).cpu().numpy(),
        ...
        return torch.cat(all_features_fnirs), torch.cat(all_features_fnirs_trans), y_all, subj_all, hbo_all, None, None, None, None, None, None, None, None, None, torch.cat(all_images_fnirs), torch.cat(all_images_fnirs_trans)
...
def CLIP_train(
    ...
    for fold_inner, (train_index, val_index) in enumerate(kf.split(dataset_train)):
        for subj in range(subject_start, 20):
            ...
            encoder_fNIRs_trans = fNIRsNet(n_channels=76, embed_dim=embed_dim, n_times=9).cuda()  # ShallowConvNet_fNIRs(input_shape=[1, 80, 9], embed_dim=embed_dim, use_norm=False).cuda()

            classifier = ReLUClassifier(input_dim=encoder_fNIRs.flatten_dim + encoder_fNIRs_trans.flatten_dim, hidden_dim=200, num_classes=20).cuda()  # define the classes for the 20 subjects
            ...
            encoder_fNIRs_trans.apply(init_weights_xavier)

            # define the optimizer here..
            optimizer = torch.optim.AdamW(list(encoder_fNIRs.parameters()) + list(encoder_fNIRs_trans.parameters()), lr=learning_rate)
            ...
            for epoch in range(0, num_epochs):
                ...
                # be sure to add this
                encoder_fNIRs_trans.train()
                encoder_fNIRs.train()

                embeddings_fnirs = []
                embeddings_fnirs_trans = []
                ...
                for eeg_imgs, fnirs_imgs, labels, subject_labels, labels_hbo, eeg_imgs_trans, fnirs_imgs_trans in train_loader:
                    ...
                    z_fnirs_trans, fnirs_flat_trans = encoder_fNIRs_trans(fnirs_features_trans.unsqueeze(1))

                    z_fnirs_norm = F.normalize(z_fnirs, dim=1)
                    z_fnirs_trans_norm = F.normalize(z_fnirs_trans, dim=1)
                    # joined the normalized features of the eeg_flat_transformation..
                    out_class = classifier(fnirs_flat, fnirs_flat_trans)

                    # append the embeddings
                    if z_fnirs_norm.shape[0] == current_batch_size:
                        embeddings_fnirs.append(z_fnirs_norm.detach().cpu().numpy())
                        embeddings_fnirs_trans.append(z_fnirs_trans_norm.detach().cpu().numpy())

                    info_nce_loss = clip_loss(z_fnirs_norm, z_fnirs_trans_norm, temperature=temperature_self)
                    info_nce_loss_class = clip_loss_conditional(z_fnirs_norm[subject_labels != subj], z_fnirs_trans_norm[subject_labels != subj], labels_hbo.cuda().long()[subject_labels != subj], temperature_class)
                ...
                del eeg_features, fnirs_features, eeg_features_trans, fnirs_features_trans, z_fnirs, z_fnirs_trans, z_fnirs_norm, z_fnirs_trans_norm
                ...
                feature_extractor_fNIRs_trans = fNIRsNetFeatureExtractor(n_channels=76)
                ...
                pretrained_dict_fNIRs_trans = encoder_fNIRs_trans.state_dict()
                feature_extractor_dict_fNIRs_trans = feature_extractor_fNIRs_trans.state_dict()
                ...
                filtered_dict_fNIRs_trans = {k: v for k, v in pretrained_dict_fNIRs_trans.items() if k in feature_extractor_dict_fNIRs_trans}
                feature_extractor_fNIRs_trans.load_state_dict(filtered_dict_fNIRs_trans)
                ...
                att_head_fNIRs_trans = AttHead(flatten_dim=feature_extractor_fNIRs_trans.flatten_dim, embed_dim=embed_dim)

                # get the weights here from the whole encoders
                att_fNIRs_dict = att_head_fNIRs.state_dict()
                att_fNIRs_dict_trans = att_head_fNIRs_trans.state_dict()
                ...
                filtered_dict_head_fNIRs_trans = {k: v for k, v in pretrained_dict_fNIRs_trans.items() if k in att_fNIRs_dict_trans}
                att_head_fNIRs_trans.load_state_dict(filtered_dict_head_fNIRs_trans)
                ...
                feat_train_fnirs, feat_train_fnirs_trans, train_labels, train_sub_labels, train_hbo_labels, _, _, _, _, _, _, _, _, _, _, _ = get_features(
                    encoder_fNIRs=encoder_fNIRs, encoder_fNIRs_trans=encoder_fNIRs_trans, dataloader=train_loader, device="cuda", channels=info.ch_names, interp_option=interp_option
                )
                if interp_option != 3:
                    feat_val_fnirs, feat_val_fnirs_trans, val_labels, val_sub_labels, val_hbo_labels, val_fnirs_occ, val_fnirs_occ_trans, sims_val, weights_fnirs_val, weights_fnirs_val_trans, SIMS_val, SIMS_soft_val, SIM_std_val, SIM_soft_val_std, _, _ = get_features(
                        encoder_fNIRs=encoder_fNIRs, encoder_fNIRs_trans=encoder_fNIRs_trans, dataloader=val_loader, do_random_occlusion=True, device="cuda", channels=info.ch_names, interp_option=interp_option
                    )
                    feat_test_fnirs, feat_test_fnirs_trans, test_labels, test_sub_labels, test_hbo_labels, test_fnirs_occ, test_fnirs_occ_trans, sims_test, weights_fnirs_test, weights_fnirs_test_trans, SIMS_test, SIMS_soft_test, SIM_std_test, SIM_soft_test_std, _, _ = (
                        get_features(encoder_fNIRs=encoder_fNIRs, encoder_fNIRs_trans=encoder_fNIRs_trans, dataloader=data_loader_test, do_random_occlusion=True, device="cuda", channels=info.ch_names, interp_option=interp_option)
                    )
                else:
                    feat_val_fnirs, feat_val_fnirs_trans, val_labels, val_sub_labels, val_hbo_labels, _, _, _, _, _, _, _, _, _, val_fnirs_imgs, val_fnirs_imgs_trans = get_features(
                        encoder_fNIRs=encoder_fNIRs, encoder_fNIRs_trans=encoder_fNIRs_trans, dataloader=val_loader, do_random_occlusion=False, device="cuda", channels=info.ch_names, interp_option=None
                    )
                    feat_test_fnirs, feat_test_fnirs_trans, test_labels, test_sub_labels, test_hbo_labels, _, _, _, _, _, _, _, _, _, test_fnirs_imgs, test_fnirs_imgs_trans = get_features(
                        encoder_fNIRs=encoder_fNIRs, encoder_fNIRs_trans=encoder_fNIRs_trans, dataloader=data_loader_test, do_random_occlusion=False, device="cuda", channels=info.ch_names, interp_option=None
                    )

                if interp_option != 3:
                    # compare the sizes of the evaluation => DO THIS FOR EVALUATING INTERPRETABILITY..
                    assert sims_val.shape[0] == val_fnirs_occ.shape[0] == val_fnirs_occ_trans.shape[0]
                    ...
                    fNIRs_xai_trans = np.zeros((76, 9))  # create image with the size of fNIRs representation

                    weights = list(zip(weights_fnirs_val, weights_fnirs_val_trans))

                    # do here the relevance calculation for occlussion analysis..
                    for n in range(val_fnirs_occ.shape[0]):
                        fnirs_2d = val_fnirs_occ[n].unsqueeze(0).cpu().numpy().transpose((1, 2, 0)).sum(axis=-1)
                        fnirs_2d_trans = val_fnirs_occ_trans[n].unsqueeze(0).cpu().numpy().transpose((1, 2, 0)).sum(axis=-1)
                        ...
                        if weight[0] < weight[1]:
                            ...
                            fNIRs_xai_trans[fnirs_2d_trans == 0] += joint_similarity
                    ...
                    grad_cam = GradCAM(encoder_EEG=feature_extractor_fNIRs, encoder_fNIRs=feature_extractor_fNIRs_trans, attentional_head_eeg=att_head_fNIRs, attentional_head_fnirs=att_head_fNIRs_trans).cuda()
                    score = grad_cam(val_fnirs_imgs.unsqueeze(1), val_fnirs_imgs_trans.unsqueeze(1))
                    ...
                    fNIRs_xai = np.mean(cv2.resize(feats_EEG, (548, 43)), axis=2)
                    fNIRs_xai_trans = np.mean(cv2.resize(feats_fNIRs, (9, 76)), axis=2)

                # do the interpretability estimation here...
                fNIRs_xai = (fNIRs_xai - np.min(fNIRs_xai)) / (np.max(fNIRs_xai) - np.min(fNIRs_xai))
                # fNIRs_xai = (fNIRs_xai - np.min(fNIRs_xai)) / (np.max(fNIRs_xai) - np.min(fNIRs_xai))
                fNIRs_xai_trans = (fNIRs_xai_trans - np.min(fNIRs_xai_trans)) / (np.max(fNIRs_xai_trans) - np.min(fNIRs_xai_trans))

                # plot here the topo-plot for the relevances across the epocs in the val dataset..
                plotting_relevance_map_eeg_fnirs(fnirs_xai_channels=np.mean(fNIRs_xai[:, :], axis=1), info_fnirs=info_fnirs, epoch=epoch, folder_imgs=folder_name_save)
                ...
                if (epoch + 1) % 10 == 0 or epoch + 1 == 1:
                    ...
                    features_tsne_ALL = tsne_map.fit_transform(
                        np.concatenate(
                            (
                                torch.cat((feat_train_fnirs, feat_train_fnirs_trans), dim=1).detach().cpu().numpy(),
                                torch.cat((feat_val_fnirs, feat_val_fnirs_trans), dim=1).detach().cpu().numpy(),
                                torch.cat((feat_test_fnirs, feat_test_fnirs_trans), dim=1).detach().cpu().numpy(),
                            ),
                            axis=0,
                        )
                    )
                    ...
                    features_UMAP_train = umap_map.fit_transform(torch.cat((feat_train_fnirs, feat_train_fnirs_trans), dim=1).detach().cpu().numpy())
                    features_UMAP_val = umap_map.transform(torch.cat((feat_val_fnirs, feat_val_fnirs_trans), dim=1).detach().cpu().numpy())
                    features_UMAP_test = umap_map.transform(torch.cat((feat_test_fnirs, feat_test_fnirs_trans), dim=1).detach().cpu().numpy())
                    

                    # GET THE T-SNE FEATURES HERE!!
                    features_tsne_train = features_tsne_ALL[0 : feat_train_fnirs.shape[0], :]
                    features_tsne_val = features_tsne_ALL[feat_train_fnirs.shape[0] : feat_train_fnirs.shape[0] + feat_val_fnirs.shape[0], :]
                    features_tsne_test = features_tsne_ALL[feat_train_fnirs.shape[0] + feat_val_fnirs.shape[0] : feat_train_fnirs.shape[0] + feat_val_fnirs.shape[0] + feat_test_fnirs.shape[0], :]
                    ...
                    wd = wasserstein_distance(
                        ...
                        np.mean(np.reshape(np.array(embeddings_fnirs_trans), (np.array(embeddings_fnirs_trans).shape[0] * current_batch_size, embed_dim)), axis=1),
                    )
                    wass_dist_vals.append(wd)

                    wd_val = wasserstein_distance(np.mean(feat_val_fnirs.detach().cpu().numpy(), axis=1), np.mean(feat_val_fnirs_trans.detach().cpu().numpy(), axis=1))
                    ...
                    # feat_test_sel = features_pca_test[labels_test_sel,:]
                    # feat_test_sel = np.concatenate((feat_test_eeg.detach().cpu().numpy()[labels_test_sel,:], feat_test_fnirs.detach().cpu().numpy()[labels_test_sel,:]), axis=1)
                    ...
                    if dfs_results_knn["acc_k1"].max() <= acc_knn_hbo_sub_1:
                        ...
                        torch.save(
                            {
                                "epoch": epoch + 1,
                                "model_state_dict": encoder_fNIRs_trans.state_dict(),
                                "optimizer_state_dict": optimizer.state_dict(),
                                "loss": loss_general,
                            },
                            f"{folder_models}/fNIRs_trans_encoder_epoch_{epoch + 1}_{subj}.pth",
                        )
            ...
            np.savez_compressed(f"{folder_models}/interpretability_measures_{subj}.npz", fNIRs_xai=fNIRs_xai)
...
os.makedirs(
    f"{save_results_path}figs_tsne_{suffix}_{embed_dim}_{batch_size}_{num_epochs}_{learning_rate}_{temperature}_stratified_cross_val_chan_remove_{suffix_interp}_{components_tsne}_{n_subs_val}_{k_value_self}_{k_value_class}_complete_class_subject_OUT_DEF_DEF_mix_more_{feat}_fix_locations_fNIRs_only",
    exist_ok=True,
)
os.makedirs(
    f"{save_results_path}models_{suffix}_{embed_dim}_{batch_size}_{num_epochs}_{learning_rate}_{temperature}_stratified_cross_val_chan_remove_{suffix_interp}_{components_tsne}_{n_subs_val}_{k_value_self}_{k_value_class}_complete_class_subject_OUT_DEF_DEF_mix_more_{feat}_fix_locations_fNIRs_only",
    exist_ok=True,
)

folder_name_save = f"{save_results_path}figs_tsne_{suffix}_{embed_dim}_{batch_size}_{num_epochs}_{learning_rate}_{temperature}_stratified_cross_val_chan_remove_{suffix_interp}_{components_tsne}_{n_subs_val}_{k_value_self}_{k_value_class}_complete_class_subject_OUT_DEF_DEF_mix_more_{feat}_fix_locations_fNIRs_only"
...
for fold, (train_idx, test_idx) in enumerate(skf.split(EEG_data, labels)):
    ...
    # define here the dataloders
    dataloader_train = DataLoader(dataset_train, batch_size=batch_size, shuffle=True, pin_memory=True)
    # dataloader_test = DataLoader(dataset_test, batch_size=100, shuffle=False, pin_memory=True)
