"""
  ...
  Usage:
    python read_fnirs_regressor.py <sel_plot> <sel_method> <channel_index> <sel_load>
...
from mne_nirs import signal_enhancement
...
def plotting_box_plot_medians(
    ...
    plt.tight_layout()
    plt.savefig("../figs_folder_auditory/median_split_" + suffix + 'regressor.jpg')
...
def plotting_average_response(data, title: str, suffix: str):
    ...
    fig.savefig('../figs_folder_auditory/' + "average_regressor_" + suffix)
...
for index_stimuli in range(0, len(stim_types)):
            ...
            if '-' in dirname and not ('_' in dirname) and sel_load == 0:
                ...
                if index_stimuli == 0:
                    color_list = plotting_time_ch_fNIRs(
                        ...
                        suffix_arch=dirname + "regressor_raw")
                ...
                # apply the short channel regression here
                dists = mne.preprocessing.nirs.source_detector_distances(raw_od.info)

                # Define your short channel threshold (e.g., 6mm or 0.6cm)
                short_channel_threshold = 0.0061  # in meters
                # Find the indices of the short channels
                short_channel_indices = [i for i, d in enumerate(dists) if d <= short_channel_threshold]
                logger.info(f"Short channels taken for regression and correction {short_channel_indices}")
                raw_od = signal_enhancement.short_channel_regression(raw_od, max_dist=short_channel_threshold)
    ...
    if sel_plot == 1:
        ...
        with open('../temporal_hrf_responses_' + f'{int(sys.argv[3])}_regressor.pkl', 'wb') as file:
...
if sel_load == 0:
    ...
    fig.savefig('../figs_folder_auditory/' +
                ...
                suffix_arch +
                f"{int(sys.argv[3])}_regressor.jpg")
...
if sel_load == 0:
    np.savez_compressed('../processed_data_' + suffix_arch + '_stim_wise_regressor.npz',
    ...
    if sel_method == 1:
        ...
        suffix_arch = "tddr"
    filename = '../DATA/fNIRs/processed_data_' + suffix_arch + '_stim_wise_regressor.npz'
